...
<template>
  <base-layout>
    <template #header>
      <div>
        <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Snapshots</p>
        <p class="mt-1 text-sm font-bold text-slate-950">Analysis details</p>
      </div>
    </template>
    ...
    <main class="mx-auto w-full max-w-7xl py-8 sm:py-10">
      <div v-if="isLoading && (!snapshot || snapshot.id !== snapshotId)" class="grid min-h-[60vh] place-items-center">
        <div class="text-center">
          ...
          <p class="mt-4 text-sm font-medium text-slate-500">Loading snapshot…</p>
        </div>
      </div>

      <template v-else-if="snapshot?.id === snapshotId">
        <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <button type="button" class="inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-indigo-700" @click="router.push({ name: 'seo-snapshot' })">
          ...
          <div class="flex flex-col justify-between gap-8 lg:flex-row lg:items-center">
            <div class="min-w-0 max-w-3xl">
              <div class="flex flex-wrap items-center gap-3">
                <span class="rounded-full bg-indigo-500/20 px-3 py-1 text-xs font-bold text-indigo-200">{{ snapshot.status || 'Complete' }}</span>
                <span class="text-xs font-medium text-slate-400">{{ formattedDate }}</span>
              </div>
              ...
              <div class="mt-6 flex flex-wrap gap-5 text-sm text-slate-400">
              ...
              <div :style="scoreStyle" class="grid size-28 place-items-center rounded-full">
                <div class="grid size-20 place-items-center rounded-full bg-slate-950 text-center">
                  <div>
                    ...
                    <p class="text-[9px] font-bold uppercase tracking-wider text-slate-500">Score</p>
                  </div>
                </div>
              </div>
              <div>
                <p class="text-xs font-bold uppercase tracking-wider text-slate-500">SEO health</p>
                <p class="mt-1 font-bold">{{ score >= 80 ? 'Strong' : score >= 60 ? 'Needs attention' : 'Needs work' }}</p>
                <p class="mt-1 max-w-32 text-xs leading-5 text-slate-400">Page-level score from this analysis.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section class="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(300px,0.65fr)]">
          <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            ...
            <h2 class="mt-2 text-2xl font-bold tracking-tight text-slate-950">Ready-to-use improvements</h2>

            <div v-if="suggestions.length" class="mt-7 space-y-4">
              <div v-for="suggestion in suggestions" :key="suggestion.label" class="rounded-2xl border border-slate-200 p-5">
                <div class="flex items-center justify-between gap-4">
                  <p class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ suggestion.label }}</p>
                  <button type="button" class="rounded-lg p-2 text-slate-400 hover:bg-indigo-50 hover:text-indigo-700" @click="copyToClipboard(suggestion.value)">
                    <span class="sr-only">Copy {{ suggestion.label }}</span>
                    <ClipboardDocumentIcon class="size-4" />
                  </button>
                </div>
                <p class="mt-3 text-sm font-medium leading-7 text-slate-700">{{ suggestion.value }}</p>
              </div>
            </div>
            <p v-else class="mt-7 rounded-2xl bg-slate-50 p-5 text-sm text-slate-500">AI recommendations are still being generated.</p>

            <div class="mt-8">
              <div class="flex items-center justify-between">
                <h3 class="font-bold text-slate-950">Recommended keywords</h3>
              ...
              <div v-if="keywords.length" class="mt-4 flex flex-wrap gap-2">
                <span v-for="keyword in keywords" :key="keyword" class="rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700">{{ keyword }}</span>
              </div>
              <p v-else class="mt-3 text-sm text-slate-500">No keyword recommendations are available yet.</p>
            </div>
          </article>
          ...
          <aside class="space-y-6">
            <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="font-bold text-slate-950">Detected issues</p>
                  <p class="text-xs text-slate-400">{{ issues.length }} item{{ issues.length === 1 ? '' : 's' }} to review</p>
                </div>
              </div>
              <ul v-if="issues.length" class="mt-5 space-y-3">
                <li v-for="issue in issues" :key="issue" class="flex gap-3 text-sm leading-6 text-slate-600">
                  <CheckIcon class="mt-1 size-4 shrink-0 text-amber-600" />
                  {{ issue }}
                </li>
              </ul>
              <p v-else class="mt-5 text-sm text-slate-500">No page-level issues were detected.</p>
            </article>
            ...
            <article class="rounded-3xl border border-rose-100 bg-rose-50/60 p-6">
              ...
              <p class="mt-2 text-xs leading-5 text-rose-700">This removes the analysis and its recommendations from your history.</p>
              <button type="button" class="mt-5 inline-flex items-center gap-2 text-sm font-bold text-rose-700 hover:text-rose-900" @click="openDelete">
      ...
      <section v-else class="rounded-3xl border border-slate-200 bg-white px-6 py-16 text-center">
        <h1 class="text-2xl font-bold text-slate-950">Snapshot not found</h1>
        <p class="mt-3 text-sm text-slate-500">This analysis may have been removed or is no longer available.</p>
      ...
      <section class="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl sm:p-8">
        <div class="flex items-start justify-between">
          <div>
            ...
            <h2 class="mt-2 text-2xl font-bold text-slate-950">Delete this snapshot?</h2>
          </div>
          <button type="button" class="rounded-lg p-2 text-slate-400 hover:bg-slate-100" @click="closeDelete"><XMarkIcon class="size-5" /></button>
        </div>
        <p class="mt-4 text-sm leading-6 text-slate-500">This cannot be undone. The snapshot and its stored recommendations will be permanently removed.</p>
        <div class="mt-7 flex justify-end gap-3">
          <button type="button" class="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold text-slate-700" @click="closeDelete">Cancel</button>
