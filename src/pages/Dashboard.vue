  ...
  <base-layout>
    <template #header>
      <div>
        ...
        <p class="mt-1 text-sm font-bold text-slate-950">Overview</p>
      </div>
    </template>

    <main class="mx-auto w-full max-w-7xl py-8 sm:py-10">
      <section class="relative overflow-hidden rounded-3xl bg-slate-950 px-6 py-8 text-white shadow-xl shadow-slate-200 sm:px-10 sm:py-10">
        <div class="absolute -right-20 -top-32 size-80 rounded-full bg-indigo-500/25 blur-3xl"></div>
        ...
        <div class="relative flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
          ...
          <div class="flex flex-col gap-3 sm:flex-row">
            <router-link
              ...
              class="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-indigo-500 px-5 text-sm font-bold text-white transition hover:bg-indigo-400"
            >
        ...
        >
          <div class="flex items-start justify-between gap-4">
            <div>
              ...
              <p class="mt-2 text-xs text-slate-400">{{ metric.detail }}</p>
            </div>
      ...
      <div v-if="isLoading && !insights" class="mt-6 rounded-3xl border border-slate-200 bg-white p-10 text-center shadow-sm">
        <div class="mx-auto size-8 animate-spin rounded-full border-2 border-slate-200 border-t-indigo-600"></div>
        <p class="mt-4 text-sm font-medium text-slate-500">Loading your latest insights…</p>
      </div>

      <section v-else-if="lastSnapshot" class="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(310px,0.75fr)]">
        <article class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          <header class="flex flex-col gap-5 border-b border-slate-100 p-6 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0">
              ...
              <h2 class="mt-3 truncate text-2xl font-bold tracking-tight text-slate-950">{{ lastSnapshot.title || hostname }}</h2>
            ...
            <router-link
              ...
              class="inline-flex shrink-0 items-center gap-2 text-sm font-bold text-indigo-600 hover:text-indigo-800"
            >
              Full analysis
          ...
          <div class="grid gap-6 p-6 lg:grid-cols-[180px_minmax(0,1fr)]">
            <div class="flex flex-col items-center justify-center rounded-2xl bg-slate-50 p-5">
              <div :style="scoreStyle" class="grid size-32 place-items-center rounded-full">
                <div class="grid size-24 place-items-center rounded-full bg-white text-center shadow-sm">
                  <div>
                    <p class="text-3xl font-black text-slate-950">{{ score }}</p>
                    <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">SEO score</p>
                  </div>
                </div>
              </div>
              <p class="mt-4 text-center text-xs leading-5 text-slate-500">Your most recent page-level result.</p>
            </div>

            <div>
              <p class="text-sm font-bold text-slate-950">Top recommendation</p>
              ...
              <div class="mt-6 grid gap-3 sm:grid-cols-2">
                <div class="rounded-2xl border border-slate-200 p-4">
                  <p class="text-xs font-bold uppercase tracking-wider text-slate-400">Suggested title</p>
                  <p class="mt-2 line-clamp-3 text-sm font-semibold leading-6 text-slate-800">
                    {{ lastSnapshot.suggestions?.improved_title || 'No title recommendation yet.' }}
                  </p>
                </div>
                <div class="rounded-2xl border border-slate-200 p-4">
                  <p class="text-xs font-bold uppercase tracking-wider text-slate-400">Suggested H1</p>
                  <p class="mt-2 line-clamp-3 text-sm font-semibold leading-6 text-slate-800">
              ...
              <div class="mt-6">
                <div class="flex items-center justify-between">
                  <p class="text-sm font-bold text-slate-950">Recommended keywords</p>
                  <router-link :to="{ name: 'keywords' }" class="text-xs font-bold text-indigo-600 hover:text-indigo-800">Explore keywords</router-link>
                </div>
                <div v-if="keywords.length" class="mt-3 flex flex-wrap gap-2">
                  <span
                    v-for="keyword in keywords.slice(0, 8)"
                    :key="keyword"
                    class="rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700"
                  >
                    {{ keyword }}
                  </span>
                </div>
                <p v-else class="mt-3 text-sm text-slate-500">Keyword recommendations will appear after analysis completes.</p>
              </div>
            </div>
          </div>
        ...
        <aside class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <p class="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Next best actions</p>
          <h2 class="mt-2 text-xl font-bold tracking-tight text-slate-950">Keep the momentum going</h2>
          ...
          <div class="mt-6 space-y-3">
            <router-link
              ...
              :class="['group block rounded-2xl border border-slate-200 p-4 transition hover:-translate-y-0.5 hover:shadow-md', action.accent]"
            >
              <div class="flex items-start gap-3">
                <component :is="action.icon" class="mt-0.5 size-5 shrink-0" />
                <div>
                  <p class="text-sm font-bold">{{ action.title }}</p>
                  <p :class="['mt-1 text-xs leading-5', action.accent.includes('bg-indigo') ? 'text-indigo-100' : 'text-slate-500']">
                    {{ action.description }}
                  </p>
                </div>
              </div>
      ...
      <section v-else class="mt-6 rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-14 text-center shadow-sm">
        ...
        <h2 class="mt-5 text-2xl font-bold tracking-tight text-slate-950">Create your first SEO snapshot</h2>
